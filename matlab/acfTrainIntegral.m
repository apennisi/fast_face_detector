function detector = acfTrainIntegral( varargin )
...
for stage = 0:numel(opts.nWeak)-1
  ...
  if( stage==0 )
    ...
    g=rgb2gray(t);
    t=chnsComputeIntegral(t, g, opts.pPyramid.pChns); 
  ...
  if( stage==0 && isempty(opts.pPyramid.lambdas) )
    ...
    ds=size(IsOrig1); 
    ds(1:end-1)=1; 
    IsOrig1=mat2cell2(IsOrig1,ds);
    ls=chnsScalingIntegral(opts.pPyramid.pChns,IsOrig1,0);
...
p=chnsPyramidIntegral([], opts.pPyramid); 
p=p.pPyramid;
...
dfs={ 'type','max', 'overlap',.65, 'ovrDnm','min' };
...
parfor i=1:n
  gray = rgb2gray(Is{i});
  C=chnsComputeIntegral(Is{i}, gray, pChns); 
