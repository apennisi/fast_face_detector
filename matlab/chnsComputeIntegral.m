function chns = chnsComputeIntegral( I, G, varargin )
...
if( ~isfield(pChns,'complete') || pChns.complete~=1 || isempty(I) )
  ...
  pChns = getPrmDflt(varargin,{'shrink',4,'pIntegral',{},'pColor',{},'pGradMag',{},...
    'pGradHist',{},'pCustom',p,'complete',1},1);

  pChns.pIntegral = getPrmDflt(pChns.pIntegral, {'type', 'upright'}, 1);
  ...
  for i=1:nc, 
      pc{i} = getPrmDflt( pChns.pCustom(i), {'enabled',1,...
      'name','REQ','hFunc','REQ','pFunc',{},'padWith',0}, 1 ); 
  end
  if( nc>0 ), 
      pChns.pCustom=[pc{:}];
  end
...
J=integralImage(G);
if(~isempty(G))
    J(1,:) = [];%eliminating the padding
    J(:,1) = [];
end
chns=addChn(chns,single(J),nm,p,0, h, w);
