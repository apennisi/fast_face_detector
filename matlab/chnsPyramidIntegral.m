function pyramid = chnsPyramidIntegral( I, varargin )
...
if( ~isfield(p,'complete') || p.complete~=1 || isempty(I) )
  ...
  chns=chnsComputeIntegral([], [], p.pChns);
  p.pChns=chns.pChns;
  p.pChns.complete=1; 
...
for i=isR
  ...
  if(all(sz==sz1)), 
      ...
      G1=imResampleMex(G,sz1(1),sz1(2),1); 
  end
  if(s==.5 && (nApprox>0 || nPerOct==1)), 
      I=I1; 
      G=G1;
  end
  chns=chnsComputeIntegral(I1, G1, pChns); 
  info=chns.info;
  if(i==isR(1)), 
      nTypes=chns.nTypes; 
      data=cell(nScales,nTypes); 
...
% minimized without changing the smaller dim of sz (tricky algebra)
    ...
    scales=scales(kp);
    scaleshw = [round(sz(1)*scales/shrink)*shrink/sz(1); round(sz(2)*scales/shrink)*shrink/sz(2)]';
end
