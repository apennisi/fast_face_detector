function [lambdas,as,scales,fs] = chnsScalingIntegral( pChns, Is, show )
...
P=chnsPyramidIntegral(Is{1},pPyramid); 
scales=P.scales'; 
info=P.info;
nScales=P.nScales; 
nTypes=P.nTypes; 
...
parfor i=1:nImages,
    P=chnsPyramidIntegral(Is{i},pPyramid);
    for j=1:nScales
        for k=1:nTypes,
...
for k=1:nTypes
  ...
  for i=round(linspace(1,nImages,20))
    loglog(1./scales1,rs(i,:,k),'Color',[1 1 1]*.8); 
    hold on; 
  end
