...
using namespace ffd;
using namespace ffd::detector;

FFDDetector::FFDDetector(const std::string &_xml_file, const float &_conf)
    : m_confidence(_conf)
{
...
FFDDetector::FFDDetector(const Classifier &_cls)
{
    m_cls = _cls;
    m_clf = _cls.cls();
...
void FFDDetector::process(const cv::Mat &_img)
{
    _img.copyTo(m_img);
...
void FFDDetector::chnsPyramid(const ffd::structs::Pyramid &_pyr)
{
    cv::Mat luv;
    float s;
    cv::Size sz;
...
void FFDDetector::chnsCompute(const cv::Mat &_img, const ffd::structs::Chns &_chns,
                               const int &_isr)
{
    nTypes = 0;
    ...
    m_padWith.push_back(structs::PadWith::FFD_NONE);
    /****END NEW PART*******/
    ...
    nTypes++;
    temp_data.push_back(data.clone());
    m_padWith.push_back(structs::PadWith::FFD_REPLICATE);

    ffd::structs::GradMag p = _chns.pGradMag;
...
cv::Mat FFDDetector::detect(const int &_i)
{
    // get inputs
    int nChns;
...
cv::Mat FFDDetector::vectorToCvMat(const std::vector<cv::Mat> &_v, int &nChannels)
{
    cv::Mat output;
