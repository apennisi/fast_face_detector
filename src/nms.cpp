    ...
    : m_type(_nms.type), m_overlap(_nms.overlap)
{
    ...
    m_radii = {.15, .15, 1, 1};
    if(_nms.ovrDnm == structs::OvrDnm::FFD_MIN)
    {
        m_ovrDnm = 0;
    }
    else if(_nms.ovrDnm == structs::OvrDnm::FFD_UNION)
    {
...
void Nms::process(std::vector<cv::Rect> &_bboxes, std::vector<float> &_confidences, const cv::Mat &_bbs)
{
    ...
    if(m_type == structs::Type::FFD_MAX)
    {
        std::cout << "1\n";
    }
    else if(m_type == structs::Type::FFD_MAXG)
    {
        maxg(_bboxes, _confidences);
    }
    else if(m_type == structs::Type::FFD_MS)
    {
        std::cout << "1\n";
    }
    else if(m_type == structs::Type::FFD_COVER)
    {
