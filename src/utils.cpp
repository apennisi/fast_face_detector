...
void RgbConvertion::process(const cv::Mat &_img, const structs::ColorSpace &_color, cv::Mat &_output)
{
    ...
    float *J;
    if(_color == structs::ColorSpace::FFD_ORIG)
    {
    ...
    else if(_color == structs::ColorSpace::FFD_LUV)
    {
        J = (float*) calloc((_img.size().area() * _img.channels()), sizeof(float));
        int d = _img.channels();
...
void GradientMag::gradMag(const cv::Mat &_img,
                          ffd::structs::GradMag p, const int &_out)
{
    cv::Mat float_image = cvMatToMat(_img).clone();
...
cv::Mat GradientMag::gradMagNorm(const cv::Mat &_img, const cv::Mat &_s, const ffd::structs::GradMag &_p)
{
    int h = _img.rows;
    int w = _img.cols;
    int d = _img.channels();
