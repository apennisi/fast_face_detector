...
namespace ffd
{
    namespace utils
    {
        ...
        class GradientMag
        {
            ...
            public:
                GradientMag();
                void gradMag(const cv::Mat &_img, ffd::structs::GradMag p, const int &_out);
                cv::Mat gradMagNorm(const cv::Mat &_img, const cv::Mat &_s, const ffd::structs::GradMag &_p);
                cv::Mat gradientHist(const cv::Mat &_img, const cv::Mat &_s, const int &_binSize,
                                     const ffd::structs::GradHist &_p, const bool &_full);
                cv::Mat M()
                {
        ...
        class ImPad
        {
            public:
                                  ...
                                  const structs::PadWith &_type, cv::Mat &_output, const float &_shrink)
                {
                    ...
                    const uint &flag = (_type == structs::PadWith::FFD_REPLICATE) ? 1 : 0;
                    const T &val = (_type == structs::PadWith::FFD_REPLICATE|| _type == structs::PadWith::FFD_NONE ) ? T(0) : T(1);
                    int pt, pb, pl, pr;
                    I = input.ptr<float>(0);
